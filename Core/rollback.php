...
if($result->count() != 0){
	foreach($result[0]['historial']->values() as $data){
		if($data->get('version')==$version){
			...
			$d2 = $data->get('version');
			print("UPDATE github.archivohist SET historial=historial+[{idArchivo:'$d1',version:'$d2',comentario:'$comentario',usuario:'$user'}] where projectname='$project' and; branchname='$branch' and docname='$file';");
			$session->execute("UPDATE github.archivohist SET historial=historial+[{idArchivo:'$d1',version:'$d2',comentario:'$comentario',usuario:'$user'}] where projectname='$project' and branchname='$branch' and docname='$file'");
			break;
		}
